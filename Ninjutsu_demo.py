...
# # Define a global variable to store jutsu_index
# jutsu_index = 0  # Initialize with the default value
...
def text_to_speech(text, lang='en'): 
    ...
    tts.save("output1.mp3")  # Save the speech to an MP3 file
    os.system("mpg123 output1.mp3")  # Play the MP3 file using a media player (mpg123 in this case)
    os.remove("output1.mp3")  # Remove the temporary MP3 file
...
def main():
    ...
    with open('setting/jutsu.csv', encoding='utf8') as f:  # technique
        jutsu = csv.reader(f)
    ...
    while True:
        ...
        if (jutsu_index != -1) and (time.time() - jutsu_start_time) < jutsu_display_time:
            if jutsu[jutsu_index][0] == '':
                jutsu_string = jutsu[jutsu_index][2 + lang_offset]
            else:
                jutsu_string = jutsu[jutsu_index][0 + lang_offset] + "：" + jutsu[jutsu_index][2 + lang_offset]

            # Convert the technique name to speech and play it
            text_to_speech(jutsu_string, lang='en' if use_jutsu_lang_en else 'ja')
...
# # Create a route to fetch jutsu_index
# @app.route('/get_jutsu_index', methods=['GET'])
# def get_jutsu_index():
#     global jutsu_index  # Access the global jutsu_index variable
#     return jsonify({'jutsu_index': jutsu_index})
